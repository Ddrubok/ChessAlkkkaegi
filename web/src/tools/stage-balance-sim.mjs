...
// 각 프로필 값은 실제 AI의 ±5도 보정 위에 더하는 결정적 추가 오차 상한이다.
 ...
 */
function collectPiecePositions(physicsRuntime, swapSides = false) {
...
/**
 * 실제 AI 순수 결정을 백 관점으로 뒤집고 프로필 추가 오차만 합성한다.
 */
function queueWhiteShot(
  ...
  const decision = modules.ai.decideAiShot(
  ...
  if (decision === null) {
    throw new Error(
      `백 ${turnIndex + 1}턴에 실제 AI 규칙이 발사 결정을 만들지 못했습니다.`,
    );
  }
  ...
  return {
    ...
    baseVariationDegrees: decision.variationDegrees,
    extraJitterDegrees,
...
function createMarkdownReport(result) {
  ...
  const normalConclusion =
    normal === undefined
      ...
      : normal.deathStageDistribution.median === null
        ? `보통 프로필은 ${normal.runCount}런 모두 패배 전에 200턴 상한에 걸렸으므로 전형적 사망 스테이지를 정할 수 없었습니다. 즉 이 표본에서 답은 특정 단계가 아니라 "측정 불가 — 1스테이지 장기 무승부"입니다.`
        : `보통 실력의 전형적 사망 스테이지는 중앙값 기준 ${normal.deathStageDistribution.median}스테이지, 최빈값 기준 ${normal.deathStageDistribution.modes.join(", ")}스테이지입니다.`;
  ...
  const forceTrendExplanation =
    trend.buffedMatchCount === 0
      ? "AI 힘 버프는 3스테이지부터 시작하지만 3스테이지에 도달한 경기가 0개라서, 힘 버프가 자멸을 늘리는지는 이 표본으로 측정할 수 없다."
      : trend.conclusion;
  const timingSeconds = result.totalWallTimeMs / 1000;
  return `# 스테이지 밸런스 자동 플레이테스트
...
이 결과는 각 프로필 ${result.configuration.runsPerProfile}런, 총 ${result.configuration.totalRuns}런의 결정적 자동 플레이 표본이다. 백은 실제 AI와 같은 최근접 규칙에 프로필별 추가 각도 오차를 더했고, 영구 강화는 0으로 두었다.
...
${forceTrendExplanation} 힘 버프가 없는 표본의 경기당 AI 자멸 평균은 ${trend.unbuffedAverageBlackSelfOuts?.toFixed(3) ?? "—"}, 힘 버프가 있는 표본은 ${trend.buffedAverageBlackSelfOuts?.toFixed(3) ?? "—"}이며 차이는 ${trend.difference?.toFixed(3) ?? "—"}이다. 힘 단계와 경기당 AI 자멸 횟수의 피어슨 상관계수는 ${trend.pearsonForceStepsVsBlackSelfOuts?.toFixed(3) ?? "—"}이다.
...
- 200턴 무승부는 총 ${result.runs.filter((run) => run.outcome === "draw-timeout").length}런이었다.
- 30스테이지 상한까지 생존한 런은 총 ${result.runs.filter((run) => run.outcome === "stage-cap").length}런이었다.
- 전체 실행에서 강제 정착은 ${result.runs.flatMap((run) => run.matches).reduce((sum, match) => sum + match.forcedSettleCount, 0)}회 발생했다.
