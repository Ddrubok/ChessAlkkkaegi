...
const resultJsonUrl = new URL(
  "./stage-balance-results-v3.json",
  import.meta.url,
);
const resultMarkdownUrl = new URL(
  "./stage-balance-results-v3.md",
  import.meta.url,
);
...
// 개발자가 지정한 3×2×3 기본 스윕 격자다.
const SWEEP_AI_JITTER_DEGREES = [5, 9, 13];
const SWEEP_AI_POWER_MAXIMUMS = [1, 0.85];
const SWEEP_BUFF_STEP_SCALES = [1, 0.5, 0.33];
// 기본 격자가 1단계 승률을 올리지 못해 허용된 6개 보정 조합에서 최대 파워를 더 낮춘다.
const REFINEMENT_DIFFICULTIES = [
  ...
  ([aiJitterDegrees, aiPowerMaximum, buffStepScale]) => ({
    id: `ref-j${aiJitterDegrees}-p${aiPowerMaximum}-b${buffStepScale}`,
    aiJitterDegrees,
    aiPowerMaximum,
    buffStepScale,
...
// 12런 표본에서 한 단계의 승률이 16.7%p 이내 역상승한 것은 두 경기 차이의 잡음으로 허용한다.
...
function queueWhiteShot(
  ...
  const decision = decideAggressiveWhiteShot(
    ...
    difficulty.playerPowerMinimum ?? 0.55,
    difficulty.playerPowerMaximum ?? 1,
  );
  if (decision === null) {
    throw new Error(
...
function prepareStage(
  ...
  const stageOptions = {
    ...
    stageNumber,
    runCards,
    ...(difficulty.useRuntimeDefaults
      ? {}
      : {
          enemyBuffStepScale:
            difficulty.buffStepScale,
          cardEffectScale:
            difficulty.cardEffectScale,
        }),
  };
  ...
  runtime.aiRuntime = modules.ai.createAiRuntime(
    ...
    () =>
      difficulty.useRuntimeDefaults
        ...
        : {
            jitterDegrees:
              difficulty.aiJitterDegrees,
            maximumPower:
              difficulty.aiPowerMaximum,
          },
    () =>
      difficulty.useRuntimeDefaults
        ? undefined
...
async function simulateAll(
  ...
  for (
    ...
    for (
      ...
      const runSeed =
        (profileSeedOffsets?.get(profile.id) ??
          profileIndex * DEFAULT_RUNS_PER_PROFILE) +
        runIndex;
...
/** 개발자가 지정한 18개 기본 난이도 조합을 고정 순서로 만든다. */
function createSweepDifficulties() {
  return SWEEP_AI_JITTER_DEGREES.flatMap(
    (aiJitterDegrees) =>
      SWEEP_AI_POWER_MAXIMUMS.flatMap((aiPowerMaximum) =>
        SWEEP_BUFF_STEP_SCALES.map((buffStepScale) => ({
          id: `j${aiJitterDegrees}-p${aiPowerMaximum}-b${buffStepScale}`,
          aiJitterDegrees,
          aiPowerMaximum,
          buffStepScale,
          useRuntimeDefaults: false,
...
/** 한 조합의 목표 지표와 단계별 승률 곡선을 요약한다. */
function summarizeSweepResult(difficulty, result) {
  const normal = result.aggregate.profiles[0];
  ...
  const monotonicWithinNoise =
    maximumPositiveBump <=
    MONOTONIC_NOISE_TOLERANCE + Number.EPSILON;
  return {
    id: difficulty.id,
    ...
    wallTimeMs: result.totalWallTimeMs,
  };
}

/** 목표에서 벗어난 정도를 가까운 조합 정렬 점수로 바꾼다. */
function computeSweepPenalty(row) {
...
/** 합격 조합 중 목표 중앙값 5.25에 가장 가까운 조합을 고른다. */
function selectSweepWinner(rows) {
  const center =
...
/** R35 결과에서 v3 비교에 필요한 원본 집계를 보존한다. */
function summarizeR35Baseline(result) {
  return {
    configuration: result.configuration,
    profiles: result.aggregate.profiles.map((profile) => ({
      ...
      drawTimeoutRuns: profile.drawTimeoutRuns,
      stageCapRuns: profile.stageCapRuns,
    })),
  };
}

/** 최종 v3 결과를 개발자가 요구한 한국어 표 보고서로 만든다. */
function createV3MarkdownReport(result) {
  const sweepRows = result.sweep.map((row) => {
    const curve = row.byStage
      ...
      .join(", ");
    return `| ${row.id} | ${row.difficulty.aiJitterDegrees}° | ${row.difficulty.aiPowerMaximum.toFixed(2)} | ${row.difficulty.buffStepScale.toFixed(2)} | ${row.medianDeathStage ?? "—"} | ${(row.stageOneWinRate * 100).toFixed(1)}% | ${(row.maximumPositiveBump * 100).toFixed(1)}%p | ${row.qualifies ? "통과" : "탈락"} | ${curve} |`;
  });
  const selection = result.selection;
  const finalProfiles =
    result.finalValidation.aggregate.profiles;
  const normal = finalProfiles.find(
    (profile) => profile.profile === "normal",
  );
  const deathRows = finalProfiles.map((profile) => {
    const distribution = profile.deathStageDistribution;
    const baseline = result.r35Baseline.profiles.find(
      (candidate) =>
        candidate.profile === profile.profile,
    );
    return `| ${profile.profileLabel} (±${profile.jitterDegrees}°) | ${distribution.median ?? "—"} | ${distribution.modes.join(", ") || "—"} | ${distribution.minimum ?? "—"}–${distribution.maximum ?? "—"} | ${profile.lossRuns}/${profile.runCount} | ${baseline?.deathStageDistribution.median ?? "—"} |`;
  });
  const allStages = [
    ...new Set(
      finalProfiles.flatMap((profile) =>
        profile.byStage.map((row) => row.stage),
      ),
    ),
  ].sort((left, right) => left - right);
  const winRows = allStages.map((stage) => {
    const cells = finalProfiles.map((profile) => {
      const row = profile.byStage.find(
        (candidate) => candidate.stage === stage,
      );
      return row === undefined
        ? "—"
        : `${row.wins}/${row.attempts} (${(row.winRate * 100).toFixed(1)}%)`;
    });
    return `| ${stage} | ${cells.join(" | ")} |`;
  });
  const normalDistribution =
    normal.deathStageDistribution.values.join(", ");
  const normalStageOne =
    normal.byStage.find((row) => row.stage === 1)
      ?.winRate ?? 0;
  const baselineNormal = result.r35Baseline.profiles.find(
    (profile) => profile.profile === "normal",
  );
  return `# 스테이지 밸런스 자동 측정 v3 — 적 AI 난이도 조정

## 1. 결론 — 선택한 상수와 최종 보통 사망 중앙값

선택한 조합은 AI 각도 오차 ±${selection.difficulty.aiJitterDegrees}°, AI 최대 파워 ${selection.difficulty.aiPowerMaximum}, 적 스테이지 중량·힘·크기 증가폭 ${selection.difficulty.buffStepScale}배이다. 최종 3프로필 검증에서 보통(±8°) 사망 스테이지 중앙값은 **${normal.deathStageDistribution.median}**, 실제 12회 분포는 **${normalDistribution}**였다. 1단계 승률은 **${(normalStageOne * 100).toFixed(1)}%**였다.

## 2. 스윕 표 (${result.sweep.length}조합)

승률 곡선은 \`단계:승/시도\`이다. 최대 상승은 인접 단계 사이 승률이 거꾸로 오른 가장 큰 폭이며, 12회 표본의 잡음 허용치는 ${(MONOTONIC_NOISE_TOLERANCE * 100).toFixed(1)}%p로 두었다.

| 조합 | AI 오차 | 최대 파워 | 버프 배율 | 사망 중앙값 | 1단계 승률 | 최대 상승 | 판정 | 단계별 승률 |
|---|---:|---:|---:|---:|---:|---:|---|---|
${sweepRows.join("\n")}

선정 기준은 사망 중앙값 ${TARGET_MEDIAN_MINIMUM}–${TARGET_MEDIAN_MAXIMUM}, 1단계 승률 ${(TARGET_STAGE_ONE_WIN_RATE * 100).toFixed(0)}% 이상, 인접 승률 상승이 표본 잡음 범위 안인 것이다. 통과 조합 중 목표 구간 중앙 5.25에 가장 가깝고 곡선 역상승이 작은 ${selection.id}를 선택했다.

## 3. 최종 검증 3프로필 표 (R35 대비)

| 프로필 | v3 사망 중앙값 | 최빈값 | 범위 | 패배/실행 | R35 중앙값 |
|---|---:|---|---|---:|---:|
${deathRows.join("\n")}
...
R35의 보통 중앙값은 ${baselineNormal.deathStageDistribution.median}였고 v3에서는 ${normal.deathStageDistribution.median}로 이동했다. 플레이어 카드·영구 강화·발사 파워는 바꾸지 않고 적 AI 기본 오차·최대 파워와 흑 스테이지 버프 증가폭만 조정했다.

## 4. 코드 변경 요약

- \`src/config.ts\`: 측정으로 선택한 AI 기본 각도 오차, AI 파워 하한·상한, 적 스테이지 버프 배율을 조정 가능한 상수로 명시했다.
- \`src/ai.ts\`: 순수 결정 함수에 선택적 오차·파워 범위 인자를 추가했다. 런타임은 인자를 생략해 설정 기본값을 쓴다.
- \`src/stage.ts\`: 흑 중량·힘·크기 단계 증가폭에만 선택적 배율을 적용했다. 백 플레이어 강화 경로는 그대로다.
- 기존 체크 스크립트는 변경된 설정값을 기대하는 검증식만 갱신했다.

## 5. 명령·결정성

\`\`\`text
node src/tools/stage-balance-sweep-v3.mjs --sweep
node src/tools/stage-balance-sweep-v3.mjs --finalize
node src/tools/stage-balance-sweep-v3.mjs --verify
npx tsc --noEmit
npm run build
\`\`\`

- 스윕: 조합당 ${result.configuration.sweepRunsPerCombo}회, ${result.sweep.length}조합
- 최종 검증: 프로필당 ${result.finalValidation.configuration.runsPerProfile}회
- 결정성 재실행은 wall-time 필드만 제외하고 최종 원시 실행·경기 결과 전체를 비교한다.

## 6. 한계

- 각 조합 12회는 방향을 잡는 작은 표본이므로 단계별 승률에는 1–2경기 규모의 흔들림이 있다.
- 공격형 자동 플레이어는 가장 밀어내기 쉬운 말을 노리지만, 사람의 연속 수 계획·위험 회피·카메라 판단은 모델링하지 않는다.
- 승률의 단조성은 유한 표본에서 완전한 수학적 단조 대신 2/12 이내의 역상승을 잡음으로 허용해 평가했다.
- 브라우저 표시·입력은 이 측정 대상이 아니며 W1–W5 회귀와 타입 검사·빌드로 코드 경로 회귀를 확인했다.
`;
}

/** 합격 조합이 없을 때 정지 근거와 가장 가까운 조합을 빠짐없이 보고한다. */
function createNoMatchMarkdownReport(result) {
  const tableRows = result.sweep.map((row) => {
    const curve = row.byStage
      .map(
        (stage) =>
          `${stage.stage}:${stage.wins}/${stage.attempts}`,
      ...
      .join(", ");
    return `| ${row.id} | ${row.difficulty.aiJitterDegrees}° | ${row.difficulty.aiPowerMaximum.toFixed(2)} | ${row.difficulty.buffStepScale.toFixed(2)} | ${row.medianDeathStage ?? "—"} | ${(row.stageOneWinRate * 100).toFixed(1)}% | ${(row.maximumPositiveBump * 100).toFixed(1)}%p | ${curve} |`;
  });
  const closestRows = result.closestThree.map(
    (row, index) =>
      `| ${index + 1} | ${row.id} | ${row.medianDeathStage ?? "—"} | ${(row.stageOneWinRate * 100).toFixed(1)}% | ${(row.maximumPositiveBump * 100).toFixed(1)}%p | ${row.byStage.map((stage) => `${stage.stage}:${stage.wins}/${stage.attempts}`).join(", ")} |`,
  );
  return `# 스테이지 밸런스 자동 측정 v3 — 적 AI 난이도 조정

## 1. 결론 — 목표 조합 없음

기본 18개와 허용된 보정 6개, 총 ${result.sweep.length}개 조합을 각 12회 측정했지만 **보통(±8°) 사망 중앙값 4.5–6.0과 1단계 승률 75% 이상을 동시에 만족한 조합이 없었다.** 최고 1단계 승률은 66.7%, 최고 사망 중앙값은 2.5였다. 지시된 정지 조건에 따라 적 난이도 상수를 강제로 적용하지 않았으며 런타임 기본 동작은 R35와 같다.
...
적용할 합격 상수가 없으므로 새 상수를 전제로 한 3프로필 최종 검증은 실행하지 않았다. 기본값은 R35와 동일하며 R35 기준 사망 중앙값은 정확 ${result.r35Baseline.profiles.find((profile) => profile.profile === "accurate").deathStageDistribution.median}, 보통 ${result.r35Baseline.profiles.find((profile) => profile.profile === "normal").deathStageDistribution.median}, 서툼 ${result.r35Baseline.profiles.find((profile) => profile.profile === "unskilled").deathStageDistribution.median}이다.
...
- 기본 스윕 wall time: ${(result.sweep.slice(0, 18).reduce((sum, row) => sum + row.wallTimeMs, 0) / 60000).toFixed(2)}분
- 보정 스윕 wall time: ${(result.sweep.slice(18).reduce((sum, row) => sum + row.wallTimeMs, 0) / 60000).toFixed(2)}분
...
const sweeps = argumentsList.includes("--sweep");
const refines = argumentsList.includes("--refine");
const finalizes = argumentsList.includes("--finalize");
const reports = argumentsList.includes("--report");
const verifiesMeasurement = argumentsList.includes(
  "--verify-measurement",
);
if (
  Number(sweeps) +
    ...
    Number(finalizes) +
    Number(reports) +
    Number(verifiesMeasurement) +
    Number(verifiesExisting) !==
  1
) {
  throw new Error(
    "--sweep, --refine, --finalize, --report, --verify-measurement, --verify 중 하나를 정확히 지정해야 합니다.",
  );
}

// 보고서 재생성은 기존 JSON만 읽으므로 Vite와 물리 모듈을 부팅하지 않는다.
if (reports) {
  const existing = JSON.parse(
    await readFile(resultJsonUrl, "utf8"),
  );
  const markdown = createNoMatchMarkdownReport(existing);
...
try {
  ...
  try {
    if (reports) {
    ...
    } else if (sweeps || refines || verifiesMeasurement) {
      const startedAt = performance.now();
      const normalProfile = SKILL_PROFILES.filter(
        (profile) => profile.id === "normal",
      );
      const seedOffsets = new Map([["normal", 12]]);
      const previous = refines || verifiesMeasurement
        ? JSON.parse(await readFile(resultJsonUrl, "utf8"))
        : null;
      const sweepRows = refines ? previous.sweep : [];
      const difficulties = verifiesMeasurement
        ? [
      ...
      for (let index = 0; index < difficulties.length; index += 1) {
        ...
        console.log(
          `[${refines ? "보정" : "스윕"} ${index + 1}/${difficulties.length}] ${difficulty.id}`,
        );
        ...
        sweepRows.push(
          summarizeSweepResult(difficulty, simulation),
        );
        const elapsed = performance.now() - startedAt;
        const projected =
          (elapsed / (index + 1)) * difficulties.length;
        const latest = sweepRows.at(-1);
        console.log(
          `[스윕 요약] 중앙값 ${latest.medianDeathStage}, 1단계 ${(latest.stageOneWinRate * 100).toFixed(1)}%, 최대 상승 ${(latest.maximumPositiveBump * 100).toFixed(1)}%p, 예상 ${(projected / 60000).toFixed(2)}분`,
        );
      }
      if (verifiesMeasurement) {
        const expected = JSON.stringify(
          stripWallTimes(previous.sweep),
        );
        const actual = JSON.stringify(
          stripWallTimes(sweepRows),
        );
        if (actual !== expected) {
          throw new Error(
            "24개 조합 결정성 재실행 결과가 기존 v3 스윕과 다릅니다.",
          );
        }
        console.log(
          `[결정성 통과] wallTimeMs 제외 24개 조합 요약 ${actual.length}자 일치`,
        );
        result = previous;
      } else {
        const selection = selectSweepWinner(sweepRows);
        result = {
        schemaVersion: 3,
        generatedBy: "stage-balance-sweep-v3.mjs",
        status:
          selection === null
            ? "no-qualifying-combination"
            : "selection-ready",
        configuration: {
          sweepRunsPerCombo: runsPerProfile,
          sweepComboCount: sweepRows.length,
          refinementComboCount: refines
            ? REFINEMENT_DIFFICULTIES.length
            : 0,
          maximumStage,
          normalProfile: normalProfile[0],
          seedOffset: 12,
        },
        target: {
          medianDeathStage: [
        ...
        sweep: sweepRows,
        selection,
        closestThree: [...sweepRows]
          .sort(
            (left, right) =>
              computeSweepPenalty(left) -
                computeSweepPenalty(right) ||
              left.id.localeCompare(right.id),
          )
          .slice(0, 3),
        r35Baseline:
          previous?.r35Baseline ??
          summarizeR35Baseline(r35Result),
        sweepWallTimeMs:
          Math.round(
      ...
      if (existing.selection === null) {
        throw new Error(
          "스윕 합격 조합이 없어 최종 검증을 실행할 수 없습니다.",
        );
      }
      ...
      for (const key of Object.keys(configured)) {
        if (configured[key] !== selected[key]) {
          throw new Error(
            `선택값과 config 불일치: ${key}, selected=${selected[key]}, config=${configured[key]}`,
          );
        }
      }
      const finalValidation = await simulateAll(
        modules,
        meta,
        geometries,
        SKILL_PROFILES,
        ...
        {
          ...configured,
          id: existing.selection.id,
          useRuntimeDefaults: true,
        },
      );
      if (verifiesExisting) {
        const expected = JSON.stringify(
          stripWallTimes(existing.finalValidation),
        );
        const actual = JSON.stringify(
          stripWallTimes(finalValidation),
        );
        if (actual !== expected) {
          ...
          while (
            ...
            actual[firstDifference] ===
              expected[firstDifference]
          ) {
            firstDifference += 1;
          }
          throw new Error(
        ...
        console.log(
          `[결정성 통과] wallTimeMs 제외 최종 검증 JSON ${actual.length}자 일치`,
        );
        result = existing;
      } else {
        result = {
  ...
  if (reports) {
    const markdown = createNoMatchMarkdownReport(result);
    await writeFile(resultMarkdownUrl, markdown, "utf8");
    console.log(
      `[저장] ${fileURLToPath(resultMarkdownUrl)} (${markdown.length}자)`,
    );
  } else if (!verifiesExisting && !verifiesMeasurement) {
    ...
    if (finalizes) {
      const markdown = createV3MarkdownReport(result);
      await writeFile(
        resultMarkdownUrl,
        markdown,
        "utf8",
    ...
    } else if (refines && result.selection === null) {
      const markdown = createNoMatchMarkdownReport(result);
      await writeFile(
        resultMarkdownUrl,
        markdown,
