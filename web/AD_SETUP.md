...
앱은 Capacitor AdMob SDK, 웹 안내 페이지는 일반 AdSense, 웹 게임 보상 광고는 AdSense H5 Games Ads를 사용합니다. 웹 광고 기본값은 `off`입니다. 코드 구현과 Google의 계정·사이트·H5 참여 승인은 별개입니다. 이 변경으로 승인이 보장되지는 않습니다.
...
5. H5 Games Ads도 별도로 신청합니다. 일반 AdSense 승인만으로 H5가 활성화되는 것은 아닙니다.
...
`display`는 안내 페이지의 광고 영역만 사용합니다. `h5`는 같은 안내 페이지 배너와 게임의 선택형 보상 광고를 사용합니다. 슬롯이 없으면 배너를 요청하지 않습니다. 게임 캔버스 옆 일반 배너는 만들지 않습니다. 전면 광고 호출 함수는 준비되어 있지만 자동 노출 호출은 추가하지 않았습니다. 우선 선택형 보상 광고를 운영하고 실제 이용 데이터를 확인하세요.
...
우선 보상형 단위만 설정해도 됩니다. 배너·전면은 사용할 때 해당 단위를 설정합니다. 프로덕션에서 누락되거나 샘플 ID인 단위는 요청하지 않습니다. 설정 뒤 웹 빌드 → `npx cap sync android` → APK/AAB 재빌드가 필요합니다. Play Console 앱 정보·데이터 보안·개인정보처리방침과 AdMob 앱 준비 상태도 확인하세요.
...
- `npm run check:ads`: SDK를 모의 처리하여 앱/웹 분리, 광고 없음·차단·취소·중복·시간 초과, 실제 +2 코인 지급과 AdMob 리스너 정리를 검사합니다. 실제 광고 요청은 하지 않습니다.
...
- 보상은 완료 이벤트에만 +2 코인, 일 5회, 60초 간격입니다. 이 횟수는 게임 규칙이며 Google의 승인 기준이 아닙니다. 취소·광고 없음·오류에는 지급하지 않고 자동 충전은 유지합니다.
- 코인 잔액은 현재 브라우저/앱 로컬 저장입니다. 광고 보상에 서버 검증이 추가된 것은 아닙니다. 현금성·교환 가능한 보상으로 확장하지 않습니다.
